        ...
        flex flex-col">
            ...
            <nav class="flex-1 p-3 space-y-2">
               ...
               {{ request()->routeIs('admin.candidates.*')
                ...
                {{-- PEMILIH (Diperbaiki ke admin.voters.index) --}}
               ...
               {{ request()->routeIs('admin.voters.*')
                ...
                {{-- VOTING / SESSION (Diperbaiki ke admin.sessions.index) --}}
               ...
               {{ request()->routeIs('admin.sessions.*')
                ...
                {{-- HASIL VOTING (FIX: Tambahkan .index agar tidak error) --}}
                <a href="{{ route('public.result.index') }}"
                    class="flex items-center gap-3 px-3 py-2 rounded-lg transition
               {{ request()->routeIs('public.result.*')
                    ? 'bg-blue-600/20 text-blue-400'
                    : 'hover:bg-white/10 text-gray-300' }}">
            ...
            <div class="p-4 border-t border-white/10 text-sm">
                ...
                <div x-show="sidebar">
                    {{-- Tambahkan @auth supaya tidak crash saat tidak login --}}
                    @auth
                    Login sebagai<br>
